...
Development Task 006 实现 OPERLOG 数据接收与解析：设备通过 `POST /iclock/cdata?table=OPERLOG` 上传操作记录时，服务器按《考勤 PUSH 通讯协议 V4.3》第 12.4 章解析并保存到 `operation_event`。

本阶段只保存设备上传的原始操作事件，不包含 USER、FACE、FINGER、BIODATA、命令队列、明道云同步或任何 ERP 业务逻辑。
...
DT007 预计根据真实设备数据继续处理下一类协议数据。DT006 完成后等待 Review，不进入 DT007。
