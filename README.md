...
Development Task 010.13 完成 Mingdao → ADMS 用户同步 API 的 PIN 分离修复：`employee_id` 保留 Mingdao/ERP 工号参考，`pin` 作为未来更新打卡机的设备用户编号，并阻止不同员工使用相同 `pin`。
...
```text
========================================
DT010.13 Mingdao PIN Support Ready
API: POST /api/v1/users
employee_id and pin are separated
Duplicate PIN returns 409 Conflict
========================================
```
