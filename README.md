...
- 验证 System Settings 下的 Inbound API 页面
- 验证 System Settings 下的 Attendance Synchronization 页面配置区和文档说明
...
DT010.11 在现有 Communication Console 中增加 System Settings 下的 Inbound API 页面：
...
该页面用于明道云考勤同步配置和手动同步。配置项包括启用状态、明道云考勤 OpenAPI URL、AppKey、Sign、Worksheet ID、Employee Record ID、Check Time、Check Date、Device Name 和 Device SN 的目标字段 ID，以及失败记录重试间隔分钟数。Test Connection 只读取明道云工作表结构，不写考勤记录。Sync Now 会将本地未同步的 `attendance_event` 调用明道 V3 新增行记录接口写入明道云。后台默认每 10 秒扫描新考勤记录并同步。系统以 `attendance_mingdao_sync.attendance_event_id` 作为本地幂等键，`SYNCED` 记录永不重复上传；缺少 `employee_record_id` 的记录保持待处理但不会计入可同步 `PENDING`，等员工档案记录 ID 补齐后再自动同步；`FAILED` 记录超过配置分钟数后由后台循环再次尝试新增。
