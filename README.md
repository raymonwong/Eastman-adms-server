...
Development Task 009.8 升级 ADMS Server Console V2：`/console` 作为多设备监控中心，支持全局设备筛选、设备名称显示、实时事件过滤、Latest Activity 和 Device Summary。Device Management 继续只负责配置。
...
Console 是开发阶段唯一控制台入口，DT009.8 后作为多设备监控中心，包含：
...
Console 只显示 Device Management 中 `show_in_console = true` 的设备。页面顶部提供全局 Device Filter，可按所有设备、位置或单台设备筛选；Dashboard、Latest Activity、Realtime Event Log 和 Device Summary 会同步跟随筛选条件。

在线状态使用最近 Heartbeat 判断：30 秒内显示 Online，超过 30 秒显示 Offline。Console 主要显示 Device Name，Device SN 只作为 tooltip 辅助信息。页面每 2 秒自动刷新，切换设备筛选时通过 `GET /console?format=json&device_filter=<value>` Ajax 刷新，不新增 `/monitor`、`/dashboard`、`/status`、`/events` 或 `/statistics` 页面。
...
DT010 预计根据真实设备数据继续处理下一类协议数据。DT009.8 完成后等待 Review，不进入 DT010。
