...
verify_console_p0() {
  ...
  compose exec -T api python -c "import json, urllib.request; data=json.loads(urllib.request.urlopen('http://127.0.0.1:8000/console?format=json&device_filter=all', timeout=5).read()); assert set(['server','filter','dashboard','heartbeat_summary','latest_activity','events','device_summary']).issubset(data); assert all(opt['value'] == 'all' or opt['value'].startswith('device:') for opt in data['filter']['options']); assert all('device_sn' in row and 'device_display' in row for row in data.get('device_summary', [])); assert all('device_sn' in item and 'device_display' in item for item in data.get('latest_activity', [])); assert all('device_sn' in event and 'device_display' in event for event in data.get('events', []))"
